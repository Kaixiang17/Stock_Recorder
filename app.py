...
# ==========================================
# 1. 頁面基本配置 (必須放在全程式第一行 Streamlit 指令)
# ==========================================
st.set_page_config(
...
def fetch_sheet_data(url):
    ...
    try:
        ...
        st.error(f"⚠️ 讀取試算表資料失敗，請確認 CSV 發布網址權限: {e}")
        return None
...
def calculate_trade_pnl(df):
    ...
    # 欄位安全整理
    qty_col = next((col for col in ["數量", "数量", "股數", "Quantity", "qty"] if col in df.columns), None)
    if qty_col:
        df["數量"] = pd.to_numeric(df[qty_col], errors="coerce").fillna(0.0).astype(float)
    else:
        df["數量"] = 0.0

    df["成交價"] = pd.to_numeric(df.get("成交價", 0), errors="coerce").fillna(0.0).astype(float)
    ...
    for idx in range(len(df)):
        ...
        if current_pnl != 0.0 or not symbol:
            continue
...
# 檢查是否有設定 Secrets
if not WEB_APP_URL or not CSV_URL:
    st.info("💡 提醒：請至 Streamlit Cloud 設定 `GOOGLE_WEB_APP_URL` 與 `GOOGLE_SHEET_CSV_URL` 即可開啓完全體儲存功能。")
    ...
    with st.form(key="trade_entry_form", clear_on_submit=True):
        ...
        if submit_button:
            if not symbol_name:
                ...
                st.error("⚠️ 請至 Streamlit Settings -> Secrets 設定 GOOGLE_WEB_APP_URL！")
            else:
                payload = {
                    "交易日期": str(trade_date),
                    "股票代號/名稱": symbol_name,
...
with tab_history:
    ...
    if not CSV_URL:
        ...
        if raw_df is None or raw_df.empty:
            st.info("💡 目前歷史資料庫為空或無法抓取資料，請確認 CSV 發布連結。")
        else:
            if "交易日期" in raw_df.columns:
                raw_df["交易日期"] = pd.to_datetime(raw_df["交易日期"], errors="coerce").dt.date
                raw_df = raw_df.sort_values(by="交易日期").reset_index(drop=True)
            
            df_calculated = calculate_trade_pnl(raw_df)
            df_calculated["月份"] = pd.to_datetime(df_calculated["交易日期"], errors="coerce").dt.strftime("%Y-%m").fillna("未知月份")
            ...
            with col_btn:
                ...
                if st.button("🔄 重新整理", use_container_width=True):
                    ...
                    st.rerun()
            with col_info:
                st.caption("💡 說明：系統先進行全歷史買賣對沖演算，再根據選取月份展現勝率與損益。")
